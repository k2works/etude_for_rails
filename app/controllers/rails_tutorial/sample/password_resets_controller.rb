class RailsTutorial::Sample::PasswordResetsController < RailsTutorial::ApplicationController
  ...
  def update
    if params[:rails_tutorial_sample_user][:password].empty?                  # (3) への対応
  ...
  def user_params
    params.require(:rails_tutorial_sample_user).permit(:password, :password_confirmation)
  end
  ...
  def check_expiration
    if @user.password_reset_expired?
      ...
      redirect_to new_rails_tutorial_sample_password_reset_url
    end
  end
end
