module Baukis
  class ProgramPresenter < ModelPresenter
    ...
    delegate :number_with_delimiter, :button_to, :baukis_customer_program_entries_path, :cancel_baukis_customer_program_entry_path, :current_customer, to: :view_context
    ...
    def apply_or_cancel_button
      if entry = object.entries.where(customer_id: current_customer.id).first
        ...
        button_to label_text, cancel_baukis_customer_program_entry_path(object), disabled: entry.canceled?, method: :patch, data: { confirm: '本当にキャンセルしますか?' }
      else
        closed = object.application_end_time < Time.current
        label_text = closed ? '募集終了' : '申し込む'
        button_to label_text, baukis_customer_program_entries_path(object), disabled: closed, method: :post, data: { confirm: '本当に申し込みますか?'}
      end
    end
  end
end