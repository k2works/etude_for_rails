class CreateSalesModelingPurchaseWarehouses < ActiveRecord::Migration[5.1]
  def change
    create_table :sales_modeling_purchase_warehouses, comment:'倉庫' do |t|
      ...
      t.references :sales_modeling_purchase_order, comment:'発注', foreign_key: true, index: {name:'index_sales_modeling_purchase_warehouses_order_id'}
      t.references :sales_modeling_purchase_stock, comment:'入庫', foreign_key: true, index: {name:'index_sales_modeling_purchase_warehouses_stock_id'}
