class CreateSalesModelingPurchaseOrders < ActiveRecord::Migration[5.1]
  def change
    create_table :sales_modeling_purchase_orders, comment:'発注' do |t|
      ...
      t.references :sales_modeling_purchase_supplier, comment: '仕入先', foreign_key: true, index: {name:'index_sales_modeling_purchase_order_supplier_id'}
      t.references :order_type_category, comment:'発注区分',index: {name:'index_sales_modeling_purchase_order_category_id'}
