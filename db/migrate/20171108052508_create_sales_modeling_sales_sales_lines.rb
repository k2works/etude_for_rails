class CreateSalesModelingSalesSalesLines < ActiveRecord::Migration[5.1]
  def change
    create_table :sales_modeling_sales_sales_lines, comment:'売上明細' do |t|
      ...
      t.references :sales_estimate,comment:'売上見積', index: {name:'index_sales_modeling_sales_on_sales_estimate_id'}
      t.references :sales_order,comment:'売上注文', index: {name:'index_sales_modeling_sales_on_sales_order_id'}
