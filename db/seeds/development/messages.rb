customers = Baukis::Customer.all
staff_members = Baukis::StaffMember.where(suspended: false).all
s = 2.years.ago
23.times do |n|
  m = Baukis::CustomerMessage.create!(
      customer: customers.sample,
      subject: 'これは問い合わせです。' * 4,
      body: "これは問い合わせです。\n" * 8,
      created_at: s.advance(months: n)
  )
  r = Baukis::StaffMessage.create!(
      customer: m.customer,
  ...
  if n % 6 == 0
    m2 = Baukis::CustomerMessage.create!(
        customer: r.customer,
        root: m,
        parent: r,
        subject: 'これは返信への返信です。',
