...
describe DesignPattern::Builder::ComputerBuilder do
  describe '#computer' do
    context 'Using DesktopBuilder' do
      ...
      it 'can build using magic method' do
        builder = DesignPattern::Builder::DeskTopBuilder.new
        builder.add_dvd_and_harddisk
        computer = builder.computer

        expect(computer.motherboard.cpu.class).to eq(DesignPattern::Builder::BasicCPU)
        expect(computer.drives.first.type).to eq(:dvd)
        expect(computer.drives.first.size).to eq(4000)
        expect(computer.drives.first.writable).to be false
        expect(computer.drives.second.type).to eq(:hard_disk)
        expect(computer.drives.second.size).to eq(100000)
        expect(computer.drives.second.writable).to be true
      end
    end
