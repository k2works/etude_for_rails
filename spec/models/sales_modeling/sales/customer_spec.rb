...
RSpec.describe SalesModeling::Sales::Customer, type: :model do
  ...
  let(:select_all) { Customer.all }
  let(:select_first) { Customer.first }
  let(:select_customer_a_by_name) {Customer.where(code:params_a[:code])}
  let(:select_customer_b_by_name) {Customer.where(code:params_b[:code])}
  let(:select_customer_c_by_name) {Customer.where(code:params_c[:code])}
  let(:select_special_customer) {Customer.where(customer_type_category:special_type)}
  let(:delete_customer_a) { Customer.destroy(select_customer_a_by_name.first.id) }
  let(:delete_customer_b) { Customer.destroy(select_customer_b_by_name.first.id) }
  let(:delete_customer_c) { Customer.destroy(select_customer_c_by_name.first.id) }
  ...
  describe '#select' do
    ...
    example '顧客A' do
      expect(select_customer_a_by_name.count).to eq 1
    end

    example '顧客B' do
      expect(select_customer_b_by_name.count).to eq 1
    end
  ...
  describe '#update' do
    ...
    example '顧客Aの顧客区分更新' do
      customer = select_customer_a_by_name.first
    ...
    example '顧客Aを顧客Bに更新' do
      ...
      expect(select_customer_b_by_name.count).to eq 2
    end

    example '顧客Cの電話番号を更新' do
      customer = select_customer_c_by_name.first
      params = params_c
      params[:telephone_number] = '333-333-3333'
      customer.update!(params)
      expect(select_customer_c_by_name.first.telephone_number).to eq '333-333-3333'
    end
  end
  ...
  describe '#delete' do
    ...
    example '顧客B,Cを削除' do
      ...
      expect(select_customer_a_by_name.count).to eq 1
      expect(select_customer_b_by_name.count).to eq 0
      expect(select_customer_c_by_name.count).to eq 0
    end
  end
end
